...
# -*- coding: utf-8 -*-
"""
Primal–Dual vs Greedy vs FIFO vs Offline (MILP)
- Adversarial arrival generator with optional Poisson/Uniform randomization
- Lookahead-enhanced Primal–Dual with adaptive rolling window (allow waiting)
- FCFS / Greedy allow waiting
- Offline MILP returns avg delay
...
def run_experiment(
   ...
   random_seed=12,
   arrival_randomization=None,  # 新增: 透传给 adversarial generator
   # ---- PD tuning knobs ----
   beta_lookahead=0.8,   # lookahead 抬价强度
   margin_factor=1.05,   # 接受阈值放大（保留容量）
   base_window=3,        # PD 基础窗口
   window_scale=6,       # 窗口随未来负荷放大
   window_bias=4,        # 窗口偏置
   greedy_window=2,      # Greedy 窗口（小）
   fcfs_window=2,        # FCFS 窗口（小）
   max_wait=None         # 若限制最大等待步数，填整数；否则 None
):
   if links is None:
   ...
   for run in range(num_runs):
       ...
       if use_adversarial_arrivals:
           all_users = generate_adversarial_users(
               ...
               arrival_randomization=arrival_randomization,  # <-   'poisson' / 'uniform' / None
               preserve_totals=True
           )
       else:
...
if __name__ == "__main__":
   ...
   # 用于保存每个规模下的 results（原始 time-series）
   results_per_size = {}

   # 存 welfare gaps 和 accepted-rate gaps
   gaps = {}      # welfare gaps (percent)
   acc_gaps = {}  # accepted-rate gaps (percent)

   # ------------------------------------------------
   # ------------------------------------------------
   for n_users in all_sizes:
       print(f"Running experiments for size = {n_users} ...")
