...
# ---------------- data source ----------------
splits = {
    'pickup_cq': 'data/pickup_cq-00000-of-00001-a172031e5392f9d3.parquet',
    'pickup_hz': 'data/pickup_hz-00000-of-00001-2641abebfe50648a.parquet',
    'pickup_jl': 'data/pickup_jl-00000-of-00001-9b430a56a935f284.parquet',
    'pickup_sh': 'data/pickup_sh-00000-of-00001-79fabe8088e723a2.parquet',
...
for _, rel in splits.items():
    d = pd.read_parquet("hf://datasets/Cainiao-AI/LaDe-P/" + rel)
    dfs.append(d)
...
KEY = [
    ...
    'accept_gps_time','pickup_gps_time',
    'ds'  #
]
missing = set(KEY) - set(df.columns)
if missing:
    raise ValueError(f"Missing necessary columns：{sorted(missing)}")

df = df.dropna(subset=KEY).copy()

# ---------------- Coordinates washing ----------------
for c in ['accept_gps_lng','accept_gps_lat','pickup_gps_lng','pickup_gps_lat']:
    df[c] = pd.to_numeric(df[c], errors='coerce')
df = df.dropna(subset=['accept_gps_lng','accept_gps_lat','pickup_gps_lng','pickup_gps_lat'])
df = df[
    df['accept_gps_lng'].between(-180, 180) &
    df['pickup_gps_lng'].between(-180, 180) &
    df['accept_gps_lat'].between(-90, 90) &
    df['pickup_gps_lat'].between(-90, 90)
].copy()
...
pic_md = pd.to_datetime(df['pickup_gps_time'].astype(str).str.strip(),  format=fmt, errors='coerce')
...
# ---------------- Eulidean distance ----------------
k = 111.32
lat_mid = np.deg2rad((df['accept_gps_lat'] + df['pickup_gps_lat']) / 2.0)
dx = (df['pickup_gps_lng'] - df['accept_gps_lng']) * k * np.cos(lat_mid)
dy = (df['pickup_gps_lat'] - df['accept_gps_lat']) * k
...
# ================= 使用示例 =================
# 你的前处理代码得到 summary_city 后，直接调用：
plot_city_comparison_styled(summary_city)
